...
class Database:
    ...
    def get_batting(self,num_pages):
        batting_list = []
        with dbapi2.connect(self.dbfile) as connection:
            cursor = connection.cursor()
            query = """SELECT playerID, yearID, teamID, lgID, R, G FROM batting
                    ORDER BY playerID
                    LIMIT 10 OFFSET ?"""
            cursor.execute(query, ((int(num_pages) - 1) * 10,))
            for playerID, yearID, teamID, lgID, R, G in cursor:
                batting_list.append(Batting(playerID, yearID, teamID, lgID, R, G))
            cursor.close()
        return batting_list
    def get_num_batting(self):
        with dbapi2.connect(self.dbfile) as connection:
            cursor = connection.cursor()
            query = "SELECT COUNT (*) FROM batting AS num_batting"
            cursor.execute(query)
            num_of_batting = cursor.fetchone()[0]
            cursor.close()
            return num_of_batting